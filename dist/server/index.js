...
var road_analyzer_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "ec9d1e977e38", "default");
//#endregion
...
var members = sqliteTable("members", {
	...
	label: text("label").notNull().default("未命名會員"),
	expiresAt: text("expires_at").notNull(),
	dailyLimit: integer("daily_limit").notNull().default(10),
	liveAnalysisEnabled: integer("live_analysis_enabled", { mode: "boolean" }).notNull().default(false),
...
sqliteTable("analysis_usage", {
	id: integer("id").primaryKey({ autoIncrement: true }),
	memberId: integer("member_id").notNull().references(() => members.id, { onDelete: "cascade" }),
	usageDate: text("usage_date").notNull(),
	count: integer("count").notNull().default(0),
	updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)
}, (table) => [uniqueIndex("analysis_usage_member_date_unique").on(table.memberId, table.usageDate), index("analysis_usage_date_idx").on(table.usageDate)]);
var trialUsage = sqliteTable("trial_usage", {
	ipHash: text("ip_hash").primaryKey(),
	ipAddress: text("ip_address").notNull().default("未記錄"),
	device: text("device").notNull().default("未知裝置"),
	count: integer("count").notNull().default(0),
	status: text("status", { enum: ["active", "disabled"] }).notNull().default("active"),
	firstUsedAt: text("first_used_at").notNull().default(sql`CURRENT_TIMESTAMP`),
	updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)
}, (table) => [index("trial_usage_status_idx").on(table.status), index("trial_usage_updated_at_idx").on(table.updatedAt)]);
sqliteTable("learning_predictions", {
	id: text("id").primaryKey(),
	memberId: integer("member_id").references(() => members.id, { onDelete: "cascade" }),
	trialIpHash: text("trial_ip_hash").references(() => trialUsage.ipHash, { onDelete: "cascade" }),
	modelVersion: text("model_version").notNull(),
	captureMode: text("capture_mode", { enum: ["upload", "live"] }).notNull(),
	predictedSide: text("predicted_side", { enum: ["B", "P"] }).notNull(),
	bankerTenths: integer("banker_tenths").notNull(),
	signalClass: text("signal_class", { enum: [
		"ask",
		"composite",
		"basic"
	] }).notNull(),
	strengthClass: text("strength_class", { enum: [
		"watch",
		"low",
		"medium"
	] }).notNull(),
	featureBucket: text("feature_bucket").notNull(),
	snapshotHash: text("snapshot_hash").notNull(),
	actualOutcome: text("actual_outcome", { enum: [
		"B",
		"P",
		"T"
	] }),
	isCorrect: integer("is_correct", { mode: "boolean" }),
	feedbackSource: text("feedback_source", { enum: ["manual", "detected-confirmed"] }),
	createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
	feedbackAt: text("feedback_at")
}, (table) => [
	index("learning_predictions_member_idx").on(table.memberId, table.createdAt),
	index("learning_predictions_trial_idx").on(table.trialIpHash, table.createdAt),
	index("learning_predictions_bucket_idx").on(table.featureBucket, table.feedbackAt),
	index("learning_predictions_created_at_idx").on(table.createdAt)
]);
//#endregion
//#region db/index.ts
function getD1() {
...
//#endregion
//#region app/device-label.ts
function deviceLabel(userAgent) {
	return `${/iPad|Tablet/i.test(userAgent) ? "平板" : /Mobile|iPhone|Android/i.test(userAgent) ? "手機" : "電腦"}・${/iPhone|iPad/i.test(userAgent) ? "iOS" : /Android/i.test(userAgent) ? "Android" : /Windows/i.test(userAgent) ? "Windows" : /Macintosh|Mac OS X/i.test(userAgent) ? "macOS" : /Linux/i.test(userAgent) ? "Linux" : "其他系統"}・${/Edg\//i.test(userAgent) ? "Edge" : /CriOS|Chrome\//i.test(userAgent) ? "Chrome" : /FxiOS|Firefox\//i.test(userAgent) ? "Firefox" : /Safari\//i.test(userAgent) ? "Safari" : "其他瀏覽器"}`;
}
//#endregion
//#region app/trial-ip.ts
...
function secret() {
	...
	if (!value) throw new Error("Trial usage protection is not configured");
	return value;
}
function hex(bytes) {
	return [...new Uint8Array(bytes)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
async function trialIpHash(headers) {
	const ip = clientIp(headers);
	if (!ip) throw new Error("Unable to identify connection address");
	const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret()), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
	return hex(await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(`trial:${ip}`)));
}
async function trialIdentity(headers) {
	const ipAddress = clientIp(headers);
	if (!ipAddress) throw new Error("Unable to identify connection address");
	return {
		ipHash: await trialIpHash(headers),
		ipAddress,
		device: deviceLabel(headers.get("user-agent") || "")
	};
}
async function trialAccess(headers) {
	const identity = await trialIdentity(headers);
	const database = getD1();
	await database.prepare(`
    INSERT INTO trial_usage (ip_hash, ip_address, device, count, status, first_used_at, updated_at)
    VALUES (?, ?, ?, 0, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ON CONFLICT(ip_hash) DO UPDATE SET
      ip_address = excluded.ip_address,
      device = excluded.device,
      updated_at = CURRENT_TIMESTAMP
  `).bind(identity.ipHash, identity.ipAddress, identity.device).run();
	const row = await database.prepare("SELECT count, status FROM trial_usage WHERE ip_hash = ? LIMIT 1").bind(identity.ipHash).first();
	return {
		used: row?.count ?? 0,
...
var metadata$2 = {
	...
	twitter: {
		...
		title: "百家樂路圖分析器｜免費牌路預測與問路工具｜MT1399",
		description: "支援手機與電腦截圖的大路及三條衍生路分析工具。"
	}
};
var structuredData = [{
	...
	mainEntity: [
		{
			"@type": "Question",
			name: "手機如何使用 MT1399 百家樂牌路分析工具？",
			acceptedAnswer: {
				"@type": "Answer",
				text: "手機使用者可依三個步驟操作：1. 在百家樂遊戲中截取包含大路與下三路的完整畫面；2. 回到 MT1399 分析器，點選選擇手機截圖並從相簿上傳；3. 辨識完成後查看大路、大眼仔路、小路與曱甴路的近期訊號整理。"
			}
		},
		{
			"@type": "Question",
			name: "MT1399 百家樂路圖分析器會讀取哪些內容？",
			acceptedAnswer: {
				"@type": "Answer",
				text: "工具會辨識截圖中的大路、大眼仔路、小路與曱甴路，整理大路近期排列、下三路規律度、訊號一致度及可辨識樣本數，再輸出保守的牌路推估；不會讀取下注金額或遊戲帳號。"
			}
		},
		{
			"@type": "Question",
			name: "電腦如何貼上百家樂牌路截圖進行分析？",
			acceptedAnswer: {
				"@type": "Answer",
				text: "Windows 可按 Win、Shift、S 截取完整牌路，回到分析器後按 Ctrl、V；也可以點擊從剪貼簿貼上或拖曳圖片。圖片需清楚包含大路與下三路。"
			}
		},
		{
			"@type": "Question",
			...
			acceptedAnswer: {
				"@type": "Answer",
				text: "只有成功辨識牌路並產生分析結果才會計算一次；無法辨識的圖片不會扣除使用次數。"
			}
		},
		{
			"@type": "Question",
			...
			acceptedAnswer: {
				"@type": "Answer",
				text: "可以。MT 真人、DG 真人、T9 真人、歐博真人、W 真人與 SA 真人等遊戲，只要畫面採用可辨識的標準百家樂牌路介面即可上傳分析；特殊版型或截圖不完整時可能無法辨識。"
			}
		},
		{
			"@type": "Question",
			name: "百家樂牌路分析結果代表下一局一定會出現嗎？",
			acceptedAnswer: {
				"@type": "Answer",
				text: "不代表。分析結果是根據已辨識牌路的近期排列、下三路一致度與樣本品質進行訊號整理，不是勝率保證；牌局結果仍具有隨機性。"
			}
		}
	]
}];
async function TrialPage() {
	...
	let disabled = false;
	try {
		const access = await trialAccess(await headers());
		used = access.used;
		disabled = access.disabled;
	} catch {
		used = 0;
	}
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("script", {
		type: "application/ld+json",
		dangerouslySetInnerHTML: { __html: JSON.stringify(structuredData).replaceAll("<", "\\u003c") }
	}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(road_analyzer_default, { trial: {
		limit: 20,
		used,
		disabled,
		lineUrl: "https://lin.ee/WG0xITo"
	} })] });
}
...
var metadata$1 = {
	...
	icons: {
		icon: [{
			url: "/mt1399-favicon-mt-v4.png",
			type: "image/png",
			sizes: "64x64"
		}],
		shortcut: "/mt1399-favicon-mt-v4.png",
		apple: "/mt1399-favicon-mt-v4.png"
	}
};
function RootLayout({ children }) {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)("html", {
		lang: "zh-Hant",
		children: /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)("body", { children: [children, /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(script_default, {
			src: `https://www.googletagmanager.com/gtag/js?id=${GA_MEASUREMENT_ID}`,
			strategy: "afterInteractive"
		}), /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(script_default, {
			id: "ga4-config",
			strategy: "afterInteractive",
...
async function sign(payload) {
	const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(runtimeSecret()), {
		name: "HMAC",
		hash: "SHA-256"
	}, false, ["sign"]);
...
async function getMemberView(memberId) {
	const today = taipeiDate();
	const row = await getD1().prepare(`
    SELECT m.id, m.username, m.label, m.expires_at AS expiresAt,
    ...
    WHERE m.id = ? AND m.status = 'active' AND m.expires_at >= ?
    LIMIT 1
  `).bind(today, memberId, today).first();
	return row ? {
		...row,
...
async function learningOwner(request) {
	const memberToken = cookieValue(request, MEMBER_COOKIE);
	if (memberToken) {
		const memberId = await memberIdFromSession(memberToken);
		if (!(memberId ? await getMemberView(memberId) : null)?.learningEnabled) return null;
		return {
			type: "member",
			memberId,
			trialIpHash: null
...
function ownerWhere(owner) {
	return owner.type === "member" ? {
		clause: "member_id = ?",
		value: owner.memberId
	} : {
		clause: "trial_ip_hash = ?",
		value: owner.trialIpHash
	};
}
//#endregion
...
async function profile() {
    ...
    SELECT feature_bucket AS featureBucket,
           COUNT(*) AS total,
           SUM(CASE WHEN actual_outcome IN ('B', 'P') THEN 1 ELSE 0 END) AS decisive,
           SUM(CASE WHEN is_correct = 1 THEN 1 ELSE 0 END) AS correct,
           SUM(CASE WHEN actual_outcome = 'T' THEN 1 ELSE 0 END) AS ties
    FROM learning_predictions
    WHERE feedback_at IS NOT NULL AND model_version = ?
    GROUP BY feature_bucket
  `).bind(MODEL_VERSION).all()).results.map((row) => ({
		...row,
		total: Number(row.total ?? 0),
		decisive: Number(row.decisive ?? 0),
		correct: Number(row.correct ?? 0),
		ties: Number(row.ties ?? 0)
	}));
	return {
		modelVersion: MODEL_VERSION,
		minimumSamples: MIN_BUCKET_SAMPLES,
		totalFeedback: rows.reduce((sum, row) => sum + row.total, 0),
		decisiveFeedback: rows.reduce((sum, row) => sum + row.decisive, 0),
		buckets: Object.fromEntries(rows.filter((row) => row.decisive >= MIN_BUCKET_SAMPLES).map((row) => [row.featureBucket, {
...
async function GET$1(request) {
	try {
		if (!await learningOwner(request)) return Response.json({ error: "目前無法使用學習功能" }, { status: 401 });
		return Response.json({ profile: await profile() }, { headers: { "Cache-Control": "no-store" } });
	} catch (error) {
		console.error("Unable to read learning profile", error);
...
async function POST$5(request) {
	try {
		...
		const body = await request.json();
		if (!isOneOf(body.predictedSide, ["B", "P"]) || !isOneOf(body.signalClass, [
			"ask",
			"composite",
			"basic"
		]) || !isOneOf(body.strengthClass, [
			"watch",
			"low",
			"medium"
		]) || !isOneOf(body.captureMode, ["upload", "live"]) || typeof body.signature !== "string" || body.signature.length < 3 || body.signature.length > 2e3) return Response.json({ error: "學習紀錄格式錯誤" }, { status: 400 });
		const bankerTenths = Number(body.bankerTenths);
		if (!Number.isInteger(bankerTenths) || bankerTenths < 450 || bankerTenths > 550) return Response.json({ error: "機率資料格式錯誤" }, { status: 400 });
		const database = getD1();
		const where = ownerWhere(owner);
		const recent = await database.prepare(`
      SELECT COUNT(*) AS count FROM learning_predictions
      WHERE ${where.clause} AND created_at >= datetime('now', '-1 minute')
    `).bind(where.value).first();
		if (Number(recent?.count ?? 0) >= 12) return Response.json({ error: "學習紀錄建立過於頻繁" }, { status: 429 });
		const featureBucket = `${body.signalClass}:${body.strengthClass}:${body.predictedSide}`;
		const id = crypto.randomUUID();
		await database.prepare(`
      INSERT INTO learning_predictions (
        id, member_id, trial_ip_hash, model_version, capture_mode, predicted_side,
        banker_tenths, signal_class, strength_class, feature_bucket, snapshot_hash
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, owner.memberId, owner.trialIpHash, MODEL_VERSION, body.captureMode, body.predictedSide, bankerTenths, body.signalClass, body.strengthClass, featureBucket, await hashSnapshot(body.signature)).run();
		return Response.json({ prediction: {
			id,
			predictedSide: body.predictedSide,
			featureBucket,
			modelVersion: MODEL_VERSION
		} }, { status: 201 });
	} catch (error) {
		console.error("Unable to create learning prediction", error);
		return Response.json({ error: "目前無法建立學習紀錄" }, { status: 503 });
	}
}
async function PATCH(request) {
	try {
		const owner = await learningOwner(request);
		if (!owner) return Response.json({ error: "目前無法回報開牌結果" }, { status: 401 });
		const body = await request.json();
		if (typeof body.predictionId !== "string" || body.predictionId.length > 80 || !isOneOf(body.actualOutcome, [
			"B",
			"P",
			"T"
		])) return Response.json({ error: "開牌結果格式錯誤" }, { status: 400 });
		const database = getD1();
		const where = ownerWhere(owner);
		const current = await database.prepare(`
      SELECT predicted_side AS predictedSide
      FROM learning_predictions
      WHERE id = ? AND ${where.clause} AND feedback_at IS NULL
      LIMIT 1
    `).bind(body.predictionId, where.value).first();
		if (!current) return Response.json({ error: "此局已回報或紀錄不存在" }, { status: 409 });
		const outcome = body.actualOutcome;
		const correct = outcome === "T" ? null : outcome === current.predictedSide;
		const updated = await database.prepare(`
      UPDATE learning_predictions
      SET actual_outcome = ?, is_correct = ?, feedback_source = ?, feedback_at = CURRENT_TIMESTAMP
      WHERE id = ? AND ${where.clause} AND feedback_at IS NULL
    `).bind(outcome, correct === null ? null : correct ? 1 : 0, body.detected === true ? "detected-confirmed" : "manual", body.predictionId, where.value).run();
		if (Number(updated.meta?.changes ?? 0) !== 1) return Response.json({ error: "此局已完成回報" }, { status: 409 });
		return Response.json({
			learned: true,
			correct,
			actualOutcome: outcome,
			predictedSide: current.predictedSide,
			profile: await profile()
		});
	} catch (error) {
		console.error("Unable to save learning feedback", error);
		return Response.json({ error: "目前無法儲存開牌結果" }, { status: 503 });
	}
}
//#endregion
...
async function GET(request) {
	try {
		const access = await trialAccess(request.headers);
		return Response.json({
			used: access.used,
			limit: 20,
			remaining: Math.max(0, 20 - access.used),
			disabled: access.disabled
		}, { headers: { "Cache-Control": "no-store" } });
	} catch {
		return Response.json({ error: "目前無法讀取免費試用局數" }, { status: 503 });
	}
}
//#endregion
//#region app/api/trial/use/route.ts
var route_exports = /* @__PURE__ */ __exportAll({ POST: () => POST });
async function POST(request) {
	try {
		const identity = await trialIdentity(request.headers);
		const database = getD1();
      ...
      ON CONFLICT(ip_hash) DO UPDATE SET
        ...
        device = excluded.device,
        updated_at = CURRENT_TIMESTAMP
      WHERE status = 'active' AND count < ?
      RETURNING count
    `).bind(identity.ipHash, identity.ipAddress, identity.device, 20).first();
		if (!result) {
			if ((await database.prepare("SELECT count, status FROM trial_usage WHERE ip_hash = ? LIMIT 1").bind(identity.ipHash).first())?.status === "disabled") return Response.json({
				error: "此 IP 的免費試用已被管理員停用",
				disabled: true
			}, { status: 403 });
			return Response.json({
				error: "免費試用次數已用完，請聯繫官方客服",
				limit: 20
			}, { status: 429 });
		}
		return Response.json({
		...
		console.error("Unable to record trial usage", error);
		return Response.json({ error: "目前無法記錄免費試用局數" }, { status: 503 });
	}
}
//#endregion
...
var routes = [
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/trial/status",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/trial/status",
		patternParts: [
			"api",
			"trial",
			"status"
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/trial/use",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/trial/use",
		patternParts: [
			"api",
			"trial",
			"use"
...
var __publicFiles = new Set([
	...
	"/file.svg",
	"/globe.svg",
	"/mt1399-favicon-full-v3.png",
	"/mt1399-favicon-mt-v4.png",
	"/mt1399-favicon-v2.png",
	"/mt1399-logo.png",
	"/og.png",
	"/robots.txt",
	"/sample-baccarat-road.png",
	"/sitemap.xml",
	"/window.svg"
]);
...
var worker_entry_default = { async fetch(request, env, ctx) {
	...
	if (url.pathname === `${BASE_PATH}/_vinext/image`) {
		...
		rewrittenUrl.pathname = "/_vinext/image";
		return handleImageOptimization(new Request(rewrittenUrl, request), {
